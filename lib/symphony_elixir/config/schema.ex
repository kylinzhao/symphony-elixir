defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule Tracker do
    ...
    def changeset(schema, attrs) do
      ...
      |> cast(
        attrs,
        [:kind, :endpoint, :api_key, :project_slug, :assignee, :active_states, :terminal_states, :app_token, :table_id, :bitable],
        empty_values: []
      )
    end
  end
  ...
  defmodule CICD do
    ...
    embedded_schema do
      ...
      field(:platform, :string, default: "github_actions")
      field(:workflow, :string)
      field(:environments, {:array, :map}, default: [])
      field(:status_mapping, :map, default: %{})
    end

    @spec changeset(%__MODULE__{}, map()) :: Ecto.Changeset.t()
    def changeset(schema, attrs) do
      schema
      |> cast(attrs, [:enabled, :platform, :workflow, :environments, :status_mapping], empty_values: [])
      |> validate_inclusion(:platform, ["github_actions", "gitlab_ci", "jenkins", "custom"])
    end
  end
