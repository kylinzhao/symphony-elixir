defmodule SymphonyElixir.AgentRunner do
  ...
  defp send_codex_update(recipient, issue, message) do
    issue_id = Map.get(issue, :id)

    if is_binary(issue_id) and is_pid(recipient) do
    ...
    if is_binary(issue_id) do
      case issue_state_fetcher.([issue_id]) do
        {:ok, [refreshed_issue | _]} when is_map(refreshed_issue) ->
          state = Map.get(refreshed_issue, :state)

          if active_issue_state?(state) do
            {:continue, refreshed_issue}
          else
